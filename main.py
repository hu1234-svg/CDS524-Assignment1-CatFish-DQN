...
# ============================================================
# Cat Eats Fish (Local UI + DQN Play)
# 目的：本地弹窗（像老师视频），并在 play 模式加载 dqn_catfish.pt 自动玩
# - mode human: 方向键/WASD 控制猫
# - mode play : DQN greedy policy 自动玩（加载你 Colab 训练出的模型）
#
# 关键：state 构造严格对齐 Colab 的 env._get_state()（11维）
# ============================================================
...
TRAP_R = 12
N_TRAPS = 5
MAX_STEPS = 900  # 需与 Colab EPISODE_MAX_STEPS 一致（你是 900）
...
def action_to_vec(a):
    # 注意：这里直接返回“像素速度”，与 Colab step 里的 self.cvx = dx*CAT_SPEED 等价
    if a == 0:
        return (0, -CAT_SPEED)
    if a == 1:
        return (0, CAT_SPEED)
